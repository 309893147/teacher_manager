<template>
  <div>
    ...
    >
      <div slot="filters">
        <template v-if="filterProperty !== 'store'">
        ...
        <el-select v-model="filter.state">
          ...
          <el-option label="未使用" value="USING"></el-option>
...
export default {
  ...
  methods: {
    ...
    reviewStore(store, status, index, reason) {
      ...
      this.ax
        .post("order" + store.id, qs.stringify(payload))
        .then(it => {
  ...
  computed: {
    tableData() {
      ...
      return this.listData.map(it => {
        ...
        it.returnName =
          it.returnPolicy === "ANYTIME" ? "随时退" : "购买后的天数";
        return it;
      });
    },
    tableHeader() {
      return [
        {
          key: "order.items[0].title",
          name: "商品名称"
        },
        {
          key: "order.orderPrice",
          name: "订单费用"
        },
        {
          key: "statusName",
